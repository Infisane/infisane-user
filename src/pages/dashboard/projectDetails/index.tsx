...
import { getSingleProject, pauseProject, resumeProject } from "../../../lib/useUser";
...
const ProjectDetails = () => {
  ...
  const totalCompletion = stagesArray.reduce((sum, [_stage, details]) => {
  ...
  const percentage =
  stagesArray.length > 0 ? totalCompletion / stagesArray.length : 0;
  
  const update = useAPIMutation({
    mutationFunction: (x: any) => single.status !== "On Hold" ? pauseProject(x.data) : resumeProject(x.data),
    onSuccessFn: (data) => {
      setLoading(false);
      toast({
        status: "success",
        description: data?.message || "Project paused successfully",
      });
            queryClient
    ...
    const handleActivity = (e: { preventDefault: () => void }, id: string) => {
      e.preventDefault();
      setLoading(true);
          ...
          <div className="p-[32px]">
            <div className="flex justify-start items-center gap-1">
              ...
              >
                ...
                <p className="text-[#808080] font-[500] text-[16px]">
                  Projects
                </p>
              </Link>
              <img src={arrow} alt="" />
            ...
            <div className="mt-4 flex justify-between w-full items-center mb-[32px]">
              ...
              <div className="flex justify-end items-center gap-4">
                <button
                  className="flex justify-center items-center gap-2 px-[32px] h-[32px] border-dark border-[2px] rounded-[4px]"
                  onClick={(e) => handleActivity(e, single._id)}
                >
                  {loading && (
                    <svg
                  ...
                  <p className="text-[12px] font-[600] text-dark">
                    {single.status === "On Hold" ? "Resume" : "Pause"}
                  </p>
                </button>
                ...
                <div className="flex items-end w-full flex-col">
                  <div className="w-[30%] bg-[#D8EBFF] p-4 rounded-tl-[25px] rounded-bl-[25px] rounded-br-[40px]">
                    <p className="text-[14px] text-[#464B4F]">
                      Good morning, i will like to change the color of the logo
                      to blue
                    </p>
                  </div>
